...
// price_current = price_original * (1 - 0.20 * weeks_elapsed); 0 at week 5+ (Section 7).
function currentPriceCents(priceOriginalCents, weeksElapsed) {
  if (weeksElapsed >= MARKDOWN_WEEKS) return 0;
  const fraction = 1 - WEEKLY_DROP_FRACTION * weeksElapsed;
  return Math.round(priceOriginalCents * fraction);
}

function isExpired(weeksElapsed) {
  return weeksElapsed >= MARKDOWN_WEEKS;
}

// Next markdown price/date, purely for display (the storefront "drops to $X on <date>"
// selling point, Section 11). Returns null once the item has hit its final markdown.
